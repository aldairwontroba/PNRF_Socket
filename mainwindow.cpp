...
/////////////////////////////////////////////////////////////////////////////
/// \brief Função Global para abrir aplicativos externos
...
// Função para converter QString para LPCSTR
LPCSTR qStringToLPCSTR(const QString &str) {
    QString modifiedStr = str;
    modifiedStr.replace("/", "\\"); // Substitui todas as ocorrências de "/" por "\\"

    QByteArray byteArray = modifiedStr.toLocal8Bit(); // Converte para um QByteArray
    char *lpcstr = new char[byteArray.size() + 1];
    strcpy(lpcstr, byteArray.constData()); // Copia os dados para o LPCSTR
...
MainWindow::~MainWindow()
{
    ...
    settings.setValue("caminhoexec", caminhoexec);
    ...
    settings.setValue("AUDIO_ENABLE", AUDIO_ENABLE);
    settings.setValue("AUDIO_AUTO_RUN", AUDIO_AUTO_RUN);
    settings.endGroup();
...
void MainWindow::restoreData()
{
    ...
    caminhoexec = settings.value("caminhoexec", QString()).toString();
    if (settings.contains("AUDIO_ENABLE")) {
        AUDIO_ENABLE = settings.value("AUDIO_ENABLE", 0).toBool();
        AUDIO_AUTO_RUN = settings.value("AUDIO_AUTO_RUN", 0).toBool();
    }else{
        AUDIO_ENABLE = false;
        AUDIO_AUTO_RUN = false;
    }
    ui->actionAuto_Run->setChecked(AUDIO_AUTO_RUN);
    ui->actionConnect_2->setChecked(AUDIO_ENABLE);
...
void MainWindow::on_actionLoad_file_triggered()
{
    ...
    file = QFileDialog::getOpenFileName(nullptr, "Selecionar .pnrf", file, "Arquivos PNRF (*.pnrf)");
        ...
        if (ATP_ISCONECTED)
        {
            ...
            if (AUDIO_ENABLE)
            {
                ...
                if (AUDIO_AUTO_RUN == 0) {
                    ...
                    caminhoexec = QFileDialog::getOpenFileName(nullptr, "Selecionar Executável", caminhoexec, "Executáveis (*.exe)");
                } else {
                    if (!caminhoexec.isEmpty()) {
                        QFileInfo fileInfo(caminhoexec);
                        if (fileInfo.suffix().toLower() != "exe") {
                            ui->textEdit->append(QString("\n Não foi selecionado um executavel\n"));
                            caminhoexec = QFileDialog::getOpenFileName(nullptr, "Selecionar Executável", caminhoexec, "Executáveis (*.exe)");
                        }
                    } else {
                        caminhoexec = QFileDialog::getOpenFileName(nullptr, "Selecionar Executável", caminhoexec, "Executáveis (*.exe)");
                ...
                timeOut = 3000;
                LPCSTR myLPCSTR = qStringToLPCSTR(caminhoexec);
                startup(myLPCSTR);
                ui->textEdit->append(QString("\n Executa: %1\n\n").arg(myLPCSTR));
            }
        }
...
void atpSimulationTr::run()
{
    ...
    {
        DWORD timeout;
        timeout  = WaitForSingleObject(workHandle, timeOut);
...
void MainWindow::on_actionAuto_Run_triggered()
{
    AUDIO_AUTO_RUN = !AUDIO_AUTO_RUN;
}
